...
I had a C++ API for accumulating amounts in a vector and then returning them in reverse, similar to this:
...
Therefore, I decided to ~resize~ the vector to a user-provided number of amounts and keep track of a countdown index. Instead of ~push_back~ amounts onto the vector, I'd assign them to indices with ~operator[]~.

However, I now have to account for mismatches between the number of amounts and the actual calls to ~append~ (both user-controlled).

Although, it might be impossible to enforce that with 100% certainty in C++, I'd be happy with making this API at least /a little/ harder to misuse.
