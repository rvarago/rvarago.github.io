...
Looking at the ~IndexedVec<T, N>~ we designed in a [[file:2025-02-11-a-length-indexed-vector-in-rust.org][previous part]], there is no built-in way to construct instances from dynamic ~Vec<T>~ because their lengths are only known at runtime.
